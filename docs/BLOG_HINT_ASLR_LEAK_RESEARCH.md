...
- 2026-05-20: Clone-assisted replay succeeded. A second identical Docker instance on `127.0.0.1:19331` was used to recover candidate `0x5555556b2167` at body offset `3136` through clone `/proc/<worker>/mem`; clone assessment artifact: [json](../artifacts/nginx_rifter_20260520-002806.json). The target on `127.0.0.1:19321` was then exploited with `tools/replay_learned_candidate.py`, deriving only target `system()` from target maps/libc and not reading target memory/core. Recorded target RCE proof: [cast](../artifacts/clone_learned_replay_rce_20260520.cast), [gif](../artifacts/clone_learned_replay_rce_20260520.gif).
